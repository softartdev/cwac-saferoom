...
public class WALTest {
        ...
        if (parentDir != null) {
            ...
            if (files != null) {
                for (File f : files) {
                    ...
                    Log.d("WALTest", "Deleted " + f.getAbsolutePath() + ": " + deleted);
                }
            }
        }
    }
    ...
    public void wal() throws IOException {
        ...
        // WAL should be disabled initially
        assertFalse(db.isWriteAheadLoggingEnabled());

        // Enable WAL
        boolean walEnabled = db.enableWriteAheadLogging();
        assertTrue("WAL should be enabled successfully", walEnabled);
        assertTrue("WAL should be enabled after calling enableWriteAheadLoggingEnabled()", db.isWriteAheadLoggingEnabled());

        db.close();

        // Reopen the database with a new factory
        options = SafeHelperFactory.Options.builder()
                .setClearPassphrase(false)
                .build();
        factory = SafeHelperFactory.fromUser(new SpannableStringBuilder(PASSPHRASE), options);
        helper = factory.create(InstrumentationRegistry.getInstrumentation().getTargetContext(), DB_NAME, new Callback(1));
        db = helper.getWritableDatabase();

        // WAL state might not persist between sessions in the new SQLCipher version
        // So we'll just verify that we can enable/disable it
        if (!db.isWriteAheadLoggingEnabled()) {
            db.enableWriteAheadLogging();
        }
        assertTrue("WAL should be enabled", db.isWriteAheadLoggingEnabled());
        
        // Disable WAL
        db.disableWriteAheadLogging();
        assertFalse("WAL should be disabled after calling disableWriteAheadLogging()", db.isWriteAheadLoggingEnabled());
